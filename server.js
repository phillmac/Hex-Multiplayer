...
// Distinct icons: ocean = 🌊 (water wave), river = 🏞️ (landscape with river)
const ICON_ABBR = {
  [TILE_TYPE.EMPTY]: '',
  [TILE_TYPE.FARM]: '🌾', [TILE_TYPE.MINE]: '⛏', [TILE_TYPE.SAWMILL]: '🪵', [TILE_TYPE.FLOUR_MILL]: '⚙',
  [TILE_TYPE.CITY]: '🏙',
  [TILE_TYPE.MOUNTAIN]: '⛰', [TILE_TYPE.RIVER]: '🏞️', [TILE_TYPE.CANYON]: '🕳', [TILE_TYPE.OCEAN]: '🌊', [TILE_TYPE.SWAMP]: '🦟',
...
function seedWorld() {
  ...
  for (const t of GAME.tiles.values()) {
    const margin = ringMargin(t.q, t.r);
    const centerBias = margin / Math.max(qHalf, rHalf, sHalf);
    const noise = randf2(t.q, t.r) * 0.35;
    t.elev = centerBias + noise;
...
function growRiverFrom(start){
  ...
  let prevDir = null;
  let steps = 0;
  const MAX_STEPS = GAME.cols * GAME.rows;
  while (cur && cur.oceanDist > 0 && steps < MAX_STEPS) {
    ...
    for (const nb of nbs) {
      const downhill = Math.max(0, (cur.elev - nb.tile.elev));
      const meander = randf2(nb.q, nb.r);
